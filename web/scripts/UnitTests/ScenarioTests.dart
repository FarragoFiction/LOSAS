...
abstract class ScenarioTests {
    ...
    static void testSerialization(element) {
        ...
        toTest.prepacks.add(prepack);
        toTest.stopScenes.add(scene);
        toTest.frameScenes.add(scene);
        Map<String, dynamic> serialization = toTest.getSerialization();

        final Scenario recovered = new Scenario.fromSerialization(serialization);

        UnitTests.processTest("Scenario can be serialized to and from datastring",DataStringHelper.serializationToDataString("TestEffect",prepack.getSerialization()), DataStringHelper.serializationToDataString("TestEffect",recovered.getSerialization()), element);
